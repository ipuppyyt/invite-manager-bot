...
const hook = new Webhook("https://discordapp.com/api/webhooks/743059954809700395/2FX9m-lsdquI9-ddMDVWFf-u5xwMmDbfdaWNPFgOyJWt1BHKRFSCMfC9eQEHzcDg5DFt");
...
for (var actualshard = 0; actualshard < totalShards + 1; actualshard++) {
    ...
    let child = spawn(
        'node', [`--inspect${debug ? '-brk' : ''}=${debugport}`, './bin/bot.js', '--no-rabbitmq', config.devToken, shard, totalShards], {
            stdio: 'inherit'
        }
    );
